...
REDIRECT_PARAMS = ["redirect", "url", "next", "return", "dest", "target", "r", "goto", "callback", "out"]
...
async def run_open_redirect_scan(requester: Requester, baseline: TargetBaseline, scan_id: str, target_url: str) -> List[Dict[str, Any]]:
    ...
    for param in REDIRECT_PARAMS[:4]:
        ...
        # Must be a redirection status code
        if res.status_code in [301, 302, 303, 307, 308]:
            headers_lower = {k.lower(): v for k, v in res.headers.items()}
            location = headers_lower.get("location", "").strip()
            
            if location:
                parsed_loc = urllib.parse.urlsplit(location)
                loc_host = parsed_loc.netloc.lower()

                # STRICT VERIFICATION ORACLE:
                # 1. The destination HOST must actually be the external attacker host
                # 2. It must NOT be a canonical/internal redirect (e.g. airbnb.com -> www.airbnb.com)
                if loc_host == ATTACKER_HOST:
                    findings.append({
                        ...
                        "technical_impact": "Can be utilized in spear-phishing campaigns and OAuth authorization code theft.",
                        "remediation": "Validate target redirection destinations against a server-side whitelist of allowed relative paths.",
                        ...
                        "verification_proof": f"HTTP {res.status_code} redirection changed destination hostname to: {loc_host}"
                    })
                    break
    return findings
